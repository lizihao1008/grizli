...
def make_templates(
    grism="G141", return_lists=False, fsps_templates=False, line_list=DEFAULT_LINE_LIST
):
    """
    ...
    fsps_templates : bool
        Use FSPS templates instead of line complex templates

    line_list : list
        List of individual lines to include in the templates
    ...
    t_complexes = utils.load_templates(
        fwhm=fwhm, line_complexes=True, fsps_templates=fsps_templates
    )
...
class StackFitter(GroupFitter):
    def __init__(
        ...
        PAs : list of int
            List of PAs to fit. If None, fit all PAs.

        extensions : list of str
            List of extensions to fit. If None, fit all extensions.

        min_ivar : float
            Minimum inverse variance threshold for fitting.
            Default = 0.01.

        overlap_threshold : float
            Threshold for masking overlapping regions.
            Default = 3.

        verbose : bool
            If True, print verbose output.

        eazyp : `~eazy.photoz.EazyPhot`
            EazyPhot object for photometric redshift fitting.

        eazy_ix : int
            Index of the EazyPhot object to use for photometric redshift fitting.
            Default = 0.
        ...
        chi2_threshold : float
            Threshold for identifying bad PAs based on chi-squared values.
            Default = 1.5 . 
        ...
        if not fit_stacks:
            ...
            if chi2_threshold > 0:
                ...
                fit_log, keep_dict, has_bad = self.check_for_bad_PAs(
                    poly_order=3,
    ...
    def check_for_bad_PAs(
        ...
        """
        Check for bad PAs and return fit log, keep dictionary, and 
        a flag indicating if there are bad PAs.

        Parameters
        ----------
        poly_order : int
            The order of the polynomial templates used for fitting. 
            Default is 1.

        chi2_threshold : float
            The threshold value for the chi-square ratio below which a PA 
            is considered good. Default is 1.5.

        fit_background : bool
            Whether to fit the background when calculating the chi-square ratio. 
            Default is True.

        reinit : bool
            Whether to reinitialize the `self` object with the beams 
            corresponding to the good PAs. Default is True.
        ...
        -------
        fit_log : OrderedDict
            A dictionary containing the chi-square values, degrees of freedom, 
            and chi-square ratios for each PA and group.

        keep_dict : OrderedDict
            A dictionary containing the list of PAs to keep for each group.
    ...
    def fit_combined_at_z(
        ...
        eazyp : `~eazy.photoz.EazyPhot`
            EazyPhot object containing the EAZY templates and filter information.
            Default in None.

        ix : int
            Index of the object in the EazyPhot object.
            Default is 0.
        ...
        full_coeffs : `~np.ndarray`
            Coefficients of the templates

        full_coeffs_err : `~np.ndarray`
        ...
        ok_phot = (
            (eazyp.efnu[ix, :] > 0)
            & (eazyp.fnu[ix, :] > eazyp.param["NOT_OBS_THRESHOLD"])
            & np.isfinite(eazyp.fnu[ix, :])
            & np.isfinite(eazyp.efnu[ix, :])
        )
        ok_phot = np.squeeze(ok_phot)
        self.ok_phot = ok_phot

        Nphot = ok_phot.sum()
        photom_eflam = (eazyp.efnu[ix, :] * eazyp.to_flam * eazyp.ext_corr / 100.0)[
            ok_phot
        ]
        photom_flam = (eazyp.fnu[ix, :] * eazyp.to_flam * eazyp.ext_corr / 100.0)[
            ok_phot
        ]
        ...
        # Photometry
        Aphot = (eazyp.tempfilt(z) * 3.0e18 / eazyp.lc ** 2 * (1 + z))[:, ok_phot]
        ...
        if scale_fit is None:
            scale_fit = scipy.optimize.minimize(
        ...
        coeffs, full, resid, chi2, AxT = self.objective_scale(
            scale_fit.x,
            Ax,
            dataf * sivarf,
            self.wavef,
    ...
    def scale_AxT(p, Ax, spec_wave, Nphot, Next):
        ...
        Nphot : int
            The number of photometric points.

        Next : int
            The number of template components.
        ...
        scale[:-Nphot] = Polynomial(p / 10.0)((spec_wave - 1.0e4) / 1000.0)
        AxT = Ax * scale
        for i in range(Next):
            AxT[i, :] /= scale
    ...
    @staticmethod
    def objective_scale(
            p, Ax, data, spec_wave, fit_mask, sivarf, Nphot, Next, return_coeffs
        ):
        """
        ...
        ----------
        p : float
            Scale term between photometry and spectra.

        Ax : `~np.ndarray`
            Array of shape (M, N) representing the design matrix.

        data : `~np.ndarray`
            Array of shape (N,) representing the observed data.

        spec_wave : `~np.ndarray`
            Array of shape (N,) representing the wavelength of the spectra.

        fit_mask : `~np.ndarray`
            Boolean array of shape (N,) representing the mask for fitting.

        sivarf : `~np.ndarray`
            Array of shape (N,) representing the inverse variance of the spectra.
        ...
        return_coeffs : bool
            Flag indicating whether to return the coefficients.
        ...
        if return_coeffs:
            ...
            coeffs : `~np.ndarray`
                Array of shape (M,) representing the coefficients of the fit.

            full : `~np.ndarray`
                Array of shape (N,) representing the full model.
            
            resid : `~np.ndarray`
                Array of shape (N,) representing the residuals.
    ...
    def fit_zgrid(
        ...
        make_plot : bool
            Make the diagnostic plot. Default is True.
        ...
        templates_file : str
            ...
            Default is 'templates.npy'.

        verbose : bool
            Print the redshift grid steps. Default is True.

        outlier_threshold : float
            Threshold for masking outliers in the fit. Default is 1e30.

        eazyp : `~eazy.photoz.EazyTemplate`
            EazyTemplate object for combined fits. Default is None.

        ix : int
            Index of the object in the eazyp object. Default is 0.

        order : int
            Order of the polynomial fit to the combined spectrum. Default is 0.

        scale_fit : float
            Scale factor for the combined spectrum. Default is None.
        ...
        for i in range(len(z)):
            if eazyp:
                out = self.fit_combined_at_z(
                    z=z[i], eazyp=eazyp, ix=ix, order=order, scale_fit=scale_fit
                )
        ...
        for iter in range(1, 7):
            ...
            for i in range(len(zi)):
                ...
                if eazyp:
                    out = self.fit_combined_at_z(
                        z=zi[i], eazyp=eazyp, ix=ix, order=order, scale_fit=scale_fit
                    )
        ...
        chi2x, bgz, fullz, coeffs, err = self.fit_at_z(
            z=z[np.argmin(chi2)], templates=t_i, fitter=fitter, get_uncertainties=True
        )
        ...
        if outlier_threshold > 0:
            ...
            print(
                "Mask {0} pixels with resid > {1} sigma".format(
        ...
        # Best-fit templates
        tc, tl = self.generate_1D_templates(coeffs, templates_file=templates_file)
        ...
        for i, te in enumerate(t_i):
            if te.startswith("line"):
                ...
                if coeffs[i] == 0:
                    ...
                    tn = t_i[te].zscale(0, scalar=coeffs[i]) + tc.zscale(0, scalar=1)
        ...
        for i in range(self.N):
            ...
            hdu.append(pyfits.ImageHDU(data=model_i, header=E.header, name="MODEL"))

            hdu.append(pyfits.ImageHDU(data=bg_i, header=E.header, name="BACKGROUND"))
    ...
    def generate_1D_templates(self, coeffs, templates_file="templates.npy"):
        """
        Generate 1D templates based on the given coefficients.
        
        Parameters
        ----------
        coeffs : array_like
            Coefficients used to scale the templates.
    ...
    def mask_drizzle_overlaps(self, threshold=3, verbose=True):
        ...
        threshold : float
            Threshold for masking. Default is 3.
        ...
        for grism in self.grisms:
            for E in self.beams:
                ...
                if grism not in min_grism:
                    ...
                    min_grism[grism][E.fit_mask] = np.minimum(
                        min_grism[grism][E.fit_mask], E.scif[E.fit_mask]
                    )
        ...
        for grism in self.grisms:
            for E in self.beams:
                ...
                if verbose:
                    print(
                        "Mask {0} additional pixels for ext {1}".format(
                            (~new_mask & E.fit_mask).sum(), E.extver
                        )
                    )
    ...
    def make_fit_plot(self, hdu, scale_pz=True, show_2d=False):
        ...
        scale_pz : bool
            Whether to scale the p(z) to DoF=1. Default is True.

        show_2d : bool
            Whether to show the 2D spectra. Default is False.
        ...
        axz.text(
            0.95,
            0.04,
            self.file + "\n" + "z={0:.4f}".format(z[np.argmin(chi2)]),
            ha="right",
            va="bottom",
        ...
        for i in range(self.N):
            ...
            fl *= unit_corr / 1.0e-19
            er *= unit_corr / 1.0e-19
            flm *= unit_corr / 1.0e-19

            f_alpha = (
                1.0 / self.Ngrism[E.grism] ** 0.5
            )  # self.h0['N{0}'.format(E.header['GRISM'])]**0.5

            axc.errorbar(
                w[clip],
                fl[clip],
                er[clip],
                color=GRISM_COLORS[E.grism],
                alpha=0.3 * f_alpha,
                marker=".",
                linestyle="None",
            )
            ...
            ymax = np.maximum(ymax, np.percentile((flm + np.median(er))[clip], 98))

            ymin = np.minimum(ymin, np.percentile((flm - er * 0.0)[clip], 2))
        ...
        for ax in [axc]:  # [axa, axb, axc]:
            ...
            labels = np.arange(np.ceil(wmin * 10), np.ceil(wmax * 10)) / 10.0
...
class StackedSpectrum(object):
    def __init__(
        self,
        file="gnt_18197.stack.G141.285.fits",
        sys_err=0.02,
        mask_min=0.1,
        extver="G141",
        mask_threshold=7,
        ...
        """
        The StackedSpectrum class.

        Parameters
        ----------
        file : str
            The path to the FITS file containing the data. 
            Default is "gnt_18197.stack.G141.285.fits".

        sys_err : float
            The systematic error to be used in the calculations. 
            Default is 0.02.

        mask_min : float
            The minimum value for the mask. Default is 0.1.

        extver : str
            The extension version. Default is "G141".

        mask_threshold : int
            The threshold value for the mask. Default is 7.

        fcontam : float
            The contamination weighting factor. Default is 1.0.

        min_ivar : float
            The minimum inverse variance value. Default is 0.001.
        ...
        self.sci = self.hdulist["SCI", extver].data * 1.0
        ...
        if ("CONTAM", extver) in self.hdulist:
            self.contam = np.abs(self.hdulist["CONTAM", extver].data * 1.0)
            ...
            self.contam = self.sci * 0.0
            self.contamf = self.scif * 0.0

            self.weight = self.sci * 0.0 + 1
            self.weightf = self.scif * 0.0 + 1
    ...
    def optimal_extract(self, data, bin=0, ivar=None, **kwargs):
        ...
        ----------
        data : `~np.ndarray`
            2D array of the data to extract.
    ...
    def _build_model(self):
        ...
        NY = self.sh[0]
        data = np.zeros(
            (self.header["NAXIS1"], self.header["NAXIS2"], self.header["NAXIS1"])
        )
        ...
        if not self.is_flambda:
            ...
            sens = interp_conserve_c(
                self.wave, conf_sens["WAVELENGTH"], conf_sens["SENSITIVITY"] * MWext
            )
            ...
            if self.grism == "G800L":
                ...
                scale = np.polynomial.Polynomial(c_i)(self.wave / 1.0e4)
                sens *= scale
    ...
    def compute_model(self, spectrum_1d=None, is_cgs=None, in_place=False):
        ...
        ----------
        spectrum_1d : tuple
            A tuple containing the wavelength and flux arrays. Default is None.

        is_cgsm, in_place : dummy parameters to match `MultiBeam.compute_model`.
